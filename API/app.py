...
@app.post('/predict', response_model=PredictionResponse) # response_model=PredictionResponse will validate first then send the user the output
...
# Keep the API in separate folder to avoid any confusion and remove unnecessary libraries from requirements.txt to make the docker image smaller.